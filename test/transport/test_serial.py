...
from amodbus.transport.serialtransport import SerialTransport, create_serial_connection


@mock.patch("amodbus.transport.serialtransport.serial.serial_for_url", mock.MagicMock())
class TestTransportSerial:
    ...
    async def test_create_serial(self):
        """Test external methods."""
        transport, protocol = await create_serial_connection(asyncio.get_running_loop(), mock.Mock, "dummy")
        assert transport
        assert protocol
        transport.close()
