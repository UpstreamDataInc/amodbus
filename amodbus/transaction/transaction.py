...
from amodbus.framer import FramerBase
from amodbus.logging import Log
from amodbus.pdu import ExceptionResponse, ModbusPDU
from amodbus.transport import CommParams, ModbusProtocol
        ...
        async with self._lock:
            ...
            while count_retries <= self.retries:
                ...
                try:
                    ...
                    raise ModbusIOException("Request cancelled outside amodbus.") from exc
            if self.count_until_disconnect < 0:
                self.connection_lost(asyncio.TimeoutError("Server not responding"))
                raise ModbusIOException(
                    "ERROR: No response received of the last requests (default: retries+3), CLOSING CONNECTION."
